...
export default function NumerasiClient({ schools }) {
  ...
  const handleExportPDF = () => {
    ...
    doc.text("Laporan Numerasi Sekolah Binaan", 105, 120, { align: 'center' });
    doc.setFontSize(16);
    ...
    schoolsToExport.forEach((school, index) => {
        ...
        if (chunks.length === 0) {
            ...
            chunks.forEach((chunk, chunkIdx) => {
                ...
                doc.autoTable({
                    head: [['No', 'Nama Siswa', 'NIS', 'Judul Tugas', 'Tanggal', 'Nilai (Bila Ada)']],
                    body: chunk,
    ...
    doc.save("Laporan_Numerasi_Massal.pdf");
    setIsModalOpen(false);
  };

  const getBadgeStyle = (status) => {
    switch(status) {
      case 'AKTIF STABIL': return { bg: '#d1fae5', text: '#059669' };
      case 'PERLU PERHATIAN': return { bg: '#fef3c7', text: '#d97706' };
      case 'KRITIS': return { bg: '#fee2e2', text: '#dc2626' };
  ...
  return (
    <div className="animate-fade-in" style={{ paddingBottom: '40px' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: '16px', marginBottom: '32px' }}>
        <div>
          <h1 style={{ fontSize: '2rem', color: '#111827', marginBottom: '8px', fontWeight: 700 }}>Monitoring Numerasi Sekolah Binaan</h1>
          <p style={{ color: '#4b5563', fontSize: '0.95rem' }}>Pantau progres pengerjaan numerasi di seluruh sekolah binaan.</p>
        </div>
        <button 
              ...
              >
                <div style={{ padding: '24px', borderBottom: '1px solid #f3f4f6' }}>
                  ...
                  <div style={{ marginBottom: '8px', display: 'flex', justifyContent: 'space-between', fontSize: '0.85rem' }}>
                    <span style={{ color: '#6b7280', fontWeight: 500 }}>Nilai Rata-Rata</span>
                ...
                <div style={{ padding: '24px', display: 'flex', justifyContent: 'space-between' }}>
                  <div>
                    <div style={{ fontSize: '0.75rem', color: '#6b7280', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: '8px' }}>Jml Tugas Selesai</div>
                    <div style={{ fontSize: '1.25rem', color: '#111827', fontWeight: 600 }}>{school.totalTasks} Tugas</div>
                  </div>
                  <div>
                    <div style={{ fontSize: '0.75rem', color: '#6b7280', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: '8px' }}>Nilai Numerasi</div>
                ...
                <div style={{ padding: '16px 24px', borderTop: '1px solid #f3f4f6', backgroundColor: '#f9fafb' }}>
                  <Link href={`/pengawas/numerasi/${school.id}`} style={{ textDecoration: 'none' }}>
      ...
      {isModalOpen && (
        <div style={{ position: 'fixed', inset: 0, backgroundColor: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100 }}>
          <div className="animate-fade-in" style={{ background: 'white', padding: '32px', borderRadius: '16px', width: '100%', maxWidth: '500px', boxShadow: '0 25px 50px -12px rgba(0,0,0,0.25)' }}>
            <h3 style={{ marginBottom: '8px', fontSize: '1.25rem', fontWeight: 700, color: '#111827' }}>Export Laporan Numerasi</h3>
            <p style={{ color: '#6b7280', fontSize: '0.9rem', marginBottom: '24px' }}>Unduh laporan penugasan numerasi siswa dalam format PDF.</p>
