...
export default function SetelanClient({ user }) {
  ...
  const confirmSave = async () => {
    ...
    try {
      const res = await fetch('/api/pengawas/settings', {
        method: 'PUT',
      ...
      if (res.ok) {
        ...
        // Kita paksa router refresh agar nama di sidebar juga ikut berubah jika memungkinkan
        // Namun auth session mungkin butuh relogin agar termutakhir. Kita beri info ringan.
    ...
    <div className="animate-fade-in" style={{ paddingBottom: '40px', maxWidth: '800px', margin: '0 auto' }}>
      <div style={{ backgroundColor: 'white', borderRadius: '12px', border: '1px solid #e5e7eb', overflow: 'hidden' }}>
        <div style={{ padding: '32px', borderBottom: '1px solid #e5e7eb', backgroundColor: '#f9fafb' }}>
          <h2 style={{ fontSize: '1.5rem', color: '#111827', margin: '0 0 8px 0', fontWeight: 700 }}>Profil Pengawas</h2>
