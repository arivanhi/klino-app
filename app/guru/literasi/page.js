...
export default async function LiterasiGuruPage() {
  ...
  school.kelas.forEach(k => {
    classData[k.id] = { id: k.id, name: k.name, students: [], totalScore: 0, scoreCount: 0, totalTasks: 0, doneTasks: 0, maxTasks: 0 };
  });
  ...
  school.siswa.forEach(student => {
    ...
    studentTasks.forEach(t => {
      ...
      if (t.score !== null || t.fileUrl !== null) {
        if (t.score !== null) totalScore += t.score;
        scoreCount++;
        if (t.score !== null) studentScoreSum += t.score;
        studentScoreCount++;
      }
    });
    ...
    if (student.kelasId && classData[student.kelasId]) {
      ...
      classData[student.kelasId].totalTasks += studentTasks.length;
      classData[student.kelasId].doneTasks += studentScoreCount;
      if (studentTasks.length > classData[student.kelasId].maxTasks) {
        classData[student.kelasId].maxTasks = studentTasks.length;
      }
    }
  });
  ...
  const classes = Object.values(classData).map(c => {
    const progress = c.totalTasks > 0 ? Math.round((c.doneTasks / c.totalTasks) * 100) : 0;
    let status = 'Sedang Berjalan';
    if (progress === 100) status = 'Selesai';
    else if (progress < 70) status = 'Membutuhkan Perhatian';

    return {
      ...c,
      progress,
      status,
      jumlahTugas: c.maxTasks
    };
  }).sort((a, b) => a.name.localeCompare(b.name));

  return <LiterasiClient schoolId={schoolId} schoolName={school.name} metrics={metrics} classes={classes} />;
}
